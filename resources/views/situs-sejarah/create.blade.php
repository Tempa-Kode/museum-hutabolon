...
@section("title-page", "Tambah Data Situs Sejarah")
@push("styles")
...
@section("body")
    <div class="row">
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('situs-sejarah.store') }}" method="POST">
                        @csrf
                        @method('POST')
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <select class="form-select js-example-basic-multiple" name="kategori_id[]" id="kategori_id" multiple="multiple">
                                    @foreach($kategori as $item)
                                        <option value="{{ $item->id }}" {{ in_array($item->id, old('kategori_id', [])) ? 'selected' : '' }}>
                                            {{ $item->nama_kategori }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                ...
                                <textarea class="form-control d-none" id="deskripsi_konten" name="deskripsi_konten" rows="3">{{ old('deskripsi_konten') }}</textarea>
                            </div>
                        </div>
                        <div class="row mb-3">
